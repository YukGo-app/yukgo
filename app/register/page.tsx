      ...
      >
            ...
            <div className="relative">
              ...
              <div className="relative flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-emerald-400 to-cyan-500 shadow-lg shadow-emerald-500/30">
                <svg viewBox="0 0 24 24" fill="none" className="h-5 w-5 text-[#05080b]">
                  <path d="M3 13l3-7h12l3 7v5h-2a2 2 0 11-4 0H9a2 2 0 11-4 0H3v-5z" stroke="currentColor" strokeWidth="1.8" strokeLinejoin="round" />
                </svg>
              </div>
            </div>
        ...
        <form onSubmit={handleSubmit} className="mt-6">
          <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-4 shadow-[0_10px_40px_-12px_rgba(16,185,129,0.15)] backdrop-blur-xl">
            <Field label="Ism" icon={<UserIcon />}>
              <input value={firstName} onChange={(event) => setFirstName(event.target.value)} placeholder="Ismingiz" autoComplete="given-name" className="w-full bg-transparent text-[15px] outline-none placeholder:text-white/30" />
            </Field>

            <div className="mt-3">
              <Field label="Familiya" icon={<UserIcon />}>
                <input value={lastName} onChange={(event) => setLastName(event.target.value)} placeholder="Familiyangiz" autoComplete="family-name" className="w-full bg-transparent text-[15px] outline-none placeholder:text-white/30" />
              </Field>
            </div>

            <div className="mt-3">
              <Field label="Email" icon={<MailIcon />}>
                <input value={email} onChange={(event) => setEmail(event.target.value)} placeholder="email@example.com" inputMode="email" autoComplete="email" className="w-full bg-transparent text-[15px] outline-none placeholder:text-white/30" />
              </Field>
            </div>

            <div className="mt-3">
              <Field label="Telefon" icon={<PhoneIcon />}>
                <input value={phone} onChange={(event) => setPhone(event.target.value)} placeholder="+998 90 123 45 67" inputMode="tel" autoComplete="tel" className="w-full bg-transparent text-[15px] outline-none placeholder:text-white/30" />
              </Field>
            </div>

            <div className="mt-3">
              <Field label="Parol" icon={<LockIcon />}>
                <input value={password} onChange={(event) => setPassword(event.target.value)} type={showPass ? "text" : "password"} placeholder="Kamida 6 ta belgi" autoComplete="new-password" className="w-full bg-transparent text-[15px] outline-none placeholder:text-white/30" />

                <button type="button" onClick={() => setShowPass((value) => !value)} className="ml-2 text-white/40 transition hover:text-white/80" aria-label="Parolni ko‘rsatish">
                  {showPass ? <EyeOff /> : <Eye />}
                </button>
              </Field>
            </div>
            ...
            <div className="mt-3">
              <Field label="Parolni tasdiqlash" icon={<CheckIcon />}>
                <input value={confirm} onChange={(event) => setConfirm(event.target.value)} type={showConfirm ? "text" : "password"} placeholder="Parolni qayta kiriting" autoComplete="new-password" className="w-full bg-transparent text-[15px] outline-none placeholder:text-white/30" />

                <button type="button" onClick={() => setShowConfirm((value) => !value)} className="ml-2 text-white/40 transition hover:text-white/80" aria-label="Parolni ko‘rsatish">
          ...
          <label className="mt-4 flex cursor-pointer items-start gap-2.5 px-1">
            ...
            <span className={`mt-0.5 flex shrink-0 items-center justify-center rounded-md border transition ${agree ? "border-emerald-300 bg-emerald-400 text-[#05080b]" : "border-white/20 bg-white/5"}`} style={{ height: "1.125rem", width: "1.125rem" }}>
              {agree ? (
            ...
            <span className="text-[12.5px] leading-relaxed text-white/55">
              Men{" "}
              <Link href="/terms" className="font-medium text-emerald-300 underline-offset-2 hover:underline">
                Foydalanish shartlari
              </Link>{" "}
              va{" "}
              <Link href="/privacy" className="font-medium text-emerald-300 underline-offset-2 hover:underline">
                Maxfiylik siyosati
              </Link>{" "}
          ...
          <button type="submit" disabled={loading || !agree} className="relative mt-5 flex h-[54px] w-full items-center justify-center overflow-hidden rounded-2xl bg-gradient-to-r from-emerald-400 to-cyan-400 font-semibold text-[#04110d] shadow-[0_12px_30px_-10px_rgba(16,185,129,0.6)] transition active:scale-[0.99] disabled:cursor-not-allowed disabled:opacity-45">
            {loading ? (
        ...
        <div className="mt-auto pt-6">
          <div className="flex items-center justify-center gap-4 text-[11.5px] text-white/40">
            <Link href="/terms" className="transition hover:text-white/70">Shartlar</Link>
            <span className="h-1 w-1 rounded-full bg-white/15" />
            <Link href="/privacy" className="transition hover:text-white/70">Maxfiylik</Link>
            <span className="h-1 w-1 rounded-full bg-white/15" />
            <Link href="/support" className="transition hover:text-white/70">Yordam</Link>
          </div>
...
function PhoneIcon() {
  return (
    <svg viewBox="0 0 24 24" fill="none" className="h-4.5 w-4.5">
      <path d="M7 4h10a2 2 0 012 2v12a2 2 0 01-2 2H7a2 2 0 01-2-2V6a2 2 0 012-2z" stroke="currentColor" strokeWidth="1.6" />
...
function ShieldIcon() {
  return (
    <svg viewBox="0 0 24 24" fill="none" className="h-4 w-4">
      <path d="M12 3l8 3v6c0 4.5-3.4 8.3-8 9-4.6-.7-8-4.5-8-9V6l8-3z" stroke="currentColor" strokeWidth="1.6" strokeLinejoin="round" />
...
function EyeOff() {
  return (
    <svg viewBox="0 0 24 24" fill="none" className="h-5 w-5">
      ...
      <path d="M10.6 6.1c.45-.07.92-.1 1.4-.1 6.5 0 10 7 10 7s-1.06 2.13-3.1 4M6.5 7.5C3.6 9.3 2 12 2 12s3.5 7 10 7c1.6 0 3-.27 4.2-.73" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" />
